...
# Restored edit route with admin access
@main_bp.route('/edit/<token>', methods=['GET', 'POST'])
def edit_preference(token):
    """Edit preference page - accessible by both Telegram users and admins"""
    # Check if the user is an admin (administrator logged in)
    if current_user.is_authenticated:
        # Admin can access - get the preference by token
    ...
    # If not an admin, redirect to access denied - Telegram users should use the telegram-specific endpoints
    return redirect(url_for('main.access_denied', reason='unauthorized'))

# Legacy thank you page - redirects to access denied for security
